...
const VideoTableRow = ({ video }) => {
  ...
  // get assignments to delete the assignment which is involved with the deleted video.
  ...
  const handleDelete = () => {
    ...
    if (relatedAssignment?.id) {
      deleteAssignment(relatedAssignment?.id);
    }
  };
  ...
  return (
    <tr>
      ...
      {showModal && (
        <AddVideoModal video={video} closeModal={handleModalToggle} />
      )}
    </tr>
  );
};
